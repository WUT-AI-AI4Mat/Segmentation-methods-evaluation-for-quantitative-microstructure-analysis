...
def parse_args():
    parser = argparse.ArgumentParser(description="Train multiclass DeepLabV3+.")
...
def main():
    ...
    print("Starting multiclass DeepLabV3+ training.", flush=True)
    ...
    for i in range(EPOCHS):
        ...
        if valid_dice > best_dice:
            ...
            epochs_no_improve = 0
            save_path = os.path.join(SAVE_DIR, 'DeepLabV3P_Best_Model.pth')
    ...
    plot_path = os.path.join(SAVE_DIR, f'deeplab_training_curve.png')
    plot_history(history, plot_path)
    print(" 所有训练任务结束！", flush=True)
