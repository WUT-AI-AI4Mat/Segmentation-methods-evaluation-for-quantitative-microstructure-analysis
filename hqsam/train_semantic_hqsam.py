...
class SemanticHQSAMDataset(Dataset):
...
class SemanticHQSAMWrapper(nn.Module):
    ...
    def forward(self, images):
        ...
        if isinstance(encoder_out, tuple):
            vision_features, interm_embeddings = encoder_out[0], encoder_out[1]
        else:
            vision_features = encoder_out
            if hasattr(self.sam_model.image_encoder, 'interm_features'):
                interm_embeddings = self.sam_model.image_encoder.interm_features
            else:
                interm_embeddings = self.sam_model.image_encoder.base_model.model.interm_features
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune HQ-SAM with semantic class tokens.")
...
def main():
    ...
    global SAVE_DIR, CHECKPOINT_PATH, NUM_CLASSES, DEVICE
    ...
    model = SemanticHQSAMWrapper(base_sam, num_classes=NUM_CLASSES).to(DEVICE)
    ...
    train_dataset = SemanticHQSAMDataset(TRAIN_IMG_DIR, TRAIN_LBL_DIR, is_train=True)
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True, collate_fn=custom_collate, num_workers=4, pin_memory=True)

    val_dataset = SemanticHQSAMDataset(VAL_IMG_DIR, VAL_LBL_DIR, is_train=False)
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in pbar:
            data = batch[0]
            loss = forward_pass(data, model, base_sam, is_train=True)
        ...
        with torch.no_grad():
            for batch in val_loader:
                data = batch[0]
                v_loss = forward_pass(data, model, base_sam, is_train=False)
        ...
        if avg_val_loss < best_loss:
            ...
            save_path = os.path.join(SAVE_DIR, "semantic_hqsam_best.pth")
