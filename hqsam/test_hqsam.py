...
def find_label_file(img_name, label_dir):
    ...
    for rule_func in LABEL_RULES:
        try:
            target_name = rule_func(base_name)
        except: continue
    ...
    return None
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate the original HQ-SAM model.")
    ...
    parser.add_argument("--stability-score-threshold", type=float, default=STABILITY_SCORE)
...
def main():
    ...
    global CHECKPOINT_PATH, POINTS_PER_SIDE, PRED_IOU_THRESH
    global STABILITY_SCORE, CROP_BOTTOM_PIXELS, DEVICE
    ...
    STABILITY_SCORE = args.stability_score_threshold
    CROP_BOTTOM_PIXELS = args.crop_bottom_pixels
    DEVICE = args.device

    print("Starting HQ-SAM evaluation.")
    ...
    for img_file in tqdm(img_files, desc="Processing"):
        ...
        try:
            ...
            if gt_label is not None:
                ...
                if h_img_curr > common_h:
                    image_rgb = image_rgb[:common_h, :]
                if h_lbl_curr > common_h:
                    gt_label = gt_label[:common_h, :]


                common_w = min(w_img_curr, w_lbl_curr)
                if w_img_curr > common_w:
                    image_rgb = image_rgb[:, :common_w]
                if w_lbl_curr > common_w:
                    gt_label = gt_label[:, :common_w]
