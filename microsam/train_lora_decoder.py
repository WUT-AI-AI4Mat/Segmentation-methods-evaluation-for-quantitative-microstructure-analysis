...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune micro-sam with LoRA and the mask decoder.")
...
class GrainPointPromptDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if len(instances) > 0:
            ...
            if len(valid_instances) > MAX_PROMPTS_PER_IMAGE:
                half_prompts = MAX_PROMPTS_PER_IMAGE // 2
                largest_indices = np.argsort(instance_areas)[-half_prompts:]
                largest_instances = valid_instances[largest_indices]

                remaining_instances = np.setdiff1d(valid_instances, largest_instances)
                num_to_sample_randomly = min(len(remaining_instances), MAX_PROMPTS_PER_IMAGE - len(largest_instances))

                if num_to_sample_randomly > 0:
                    random_instances = np.random.choice(remaining_instances, num_to_sample_randomly, replace=False)
                    sampled_instances = np.concatenate([largest_instances, random_instances])
                else:
                    sampled_instances = largest_instances
            else:
                sampled_instances = valid_instances

            for inst_id in sampled_instances:
...
def forward_pass(data, model, is_train=True):
    ...
    with torch.set_grad_enabled(is_train):
        ...
        pred_masks = F.interpolate(
            low_res_masks, size=original_size, mode="bilinear", align_corners=False,
        )
...
def main():
    ...
    global SAVE_DIR, CHECKPOINT_PATH, MODEL_TYPE, DEVICE
    ...
    print("Starting micro-sam LoRA and mask-decoder fine-tuning.")
    ...
    for param in sam_model.mask_decoder.parameters(): param.requires_grad = True
    ...
    for epoch in range(EPOCHS):
        ...
        for batch in pbar:
            data = batch[0]
            loss = forward_pass(data, sam_model, is_train=True)
        ...
        with torch.no_grad():
            for batch in val_loader:
                data = batch[0]
                v_loss = forward_pass(data, sam_model, is_train=False)
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_path = os.path.join(SAVE_DIR, "microsam_lora_decoder_best.pth")
            trainable_state_dict = {k: v.cpu() for k, v in sam_model.named_parameters() if v.requires_grad}
            torch.save(trainable_state_dict, save_path)
        else:
            no_improve_count += 1
            status_msg += f" | early stopping: {no_improve_count}/{patience}"
