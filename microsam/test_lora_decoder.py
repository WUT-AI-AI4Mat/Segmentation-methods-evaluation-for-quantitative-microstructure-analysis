...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate LoRA and mask-decoder fine-tuned micro-sam.")
    ...
    parser.add_argument("--center-distance-threshold", type=float, default=MICROSAM_PARAMS["center_distance_threshold"])
    parser.add_argument("--boundary-distance-threshold", type=float, default=MICROSAM_PARAMS["boundary_distance_threshold"])
...
def main():
    ...
    global MODEL_TYPE, CHECKPOINT_PATH, FINETUNED_WEIGHT_PATH, DEVICE
    global CROP_BOTTOM_PIXELS, MAX_MASK_AREA, MICROSAM_PARAMS
    ...
    print("Starting fine-tuned micro-sam evaluation.")
    ...
    if os.path.exists(FINETUNED_WEIGHT_PATH):
        ...
        predictor.model.image_encoder = get_peft_model(predictor.model.image_encoder, lora_config)
        loaded_count = load_incremental_checkpoint(
            predictor.model, FINETUNED_WEIGHT_PATH, map_location=DEVICE
        )
        predictor.model.image_encoder = predictor.model.image_encoder.merge_and_unload()
    ...
    for img_file in tqdm(img_files, desc="Processing"):
        ...
        try:
            ...
            if gt_label is not None:
                ...
                if CROP_BOTTOM_PIXELS > 0 and abs(h_lbl_origin - h_raw_origin) < 2:
