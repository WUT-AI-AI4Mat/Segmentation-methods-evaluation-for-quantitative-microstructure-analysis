...
try:
    ...
    print("Loaded micro-sam and evaluation dependencies.")
...
def cv_imread(file_path, flags=cv2.IMREAD_COLOR):
    ...
    if img is None and file_path.lower().endswith(('.tif', '.tiff')):
        try:
            ...
            if flags == cv2.IMREAD_COLOR and img.ndim == 3: img = cv2.cvtColor(img, cv2.COLOR_RGB2BGR)
        except: pass
...
def parse_args():
    parser = argparse.ArgumentParser(description="Evaluate class-token fine-tuned micro-sam.")
...
def main():
    ...
    global CHECKPOINT_PATH, FINETUNED_WEIGHT_PATH, MODEL_TYPE, NUM_CLASSES, DEVICE
    ...
    model = SemanticMicroSAMWrapper(base_sam, num_classes=NUM_CLASSES).to(DEVICE)
