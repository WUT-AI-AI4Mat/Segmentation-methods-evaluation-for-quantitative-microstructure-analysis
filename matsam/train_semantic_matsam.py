...
class SemanticMatSAMDataset(Dataset):
...
def parse_args():
    parser = argparse.ArgumentParser(description="Fine-tune MatSAM with semantic class tokens.")
...
def main():
    ...
    print("Starting MatSAM semantic class-token fine-tuning.")
    is_terminal = sys.stdout.isatty()

    sam_base = sam_model_registry[MODEL_TYPE](checkpoint=CHECKPOINT_PATH).to(DEVICE)
    ...
    model = SemanticMatSAMWrapper(sam_base, num_classes=NUM_CLASSES).to(DEVICE)
    ...
    train_dataset = SemanticMatSAMDataset(TRAIN_IMG_DIR, TRAIN_LBL_DIR, is_train=True)
    train_loader = DataLoader(train_dataset, batch_size=1, shuffle=True, collate_fn=custom_collate, num_workers=4, pin_memory=True)

    val_dataset = SemanticMatSAMDataset(VAL_IMG_DIR, VAL_LBL_DIR, is_train=False)
    ...
    for epoch in range(EPOCHS):
        ...
        if avg_val_loss < best_val_loss:
            ...
            save_path = os.path.join(SAVE_DIR, "semantic_matsam_best.pth")
